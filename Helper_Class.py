...
class Game:
    ...
    def learn_Q(self, move):                        # If Q-learning is toggled on, "learn_Q" should be called after receiving a move from an instance of Player and before implementing the move (using Board's "place_mark" method)
        ...
        if next_board.over():
            expected = reward
            if expected == -1:
                self.win_rates['O'].append(1)
                self.win_rates['X'].append(0)
            elif expected == 1:
                self.win_rates['X'].append(1)
                self.win_rates['O'].append(0)
            else:
                self.win_rates['X'].append(0.5)
                self.win_rates['O'].append(0.5)
        else:
...
class Board:
    ...
    def over(self):             # The game is over if there is a winner or if no squares remain empty (draw)
    ...
    def give_reward(self):                          # Assign a reward for the player with mark X in the current board position.
        if self.over():
            if self.winner() is not None:
                ...
                return 0.5                          # A smaller positive reward for draw
        else:
...
class RandomPlayer(ComputerPlayer):
    ...
    def get_move(board):
        ...
        if moves:   # If "moves" is not an empty list (as it would be if draw were reached)
